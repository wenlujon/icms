...
function update_user_caches($user) {
	...
	wp_cache_add($user->display_name, $user->ID, 'userdisplayname');
}
...
function clean_user_cache( $user ) {
	...
	wp_cache_delete( $user->display_name, 'userdisplayname' );
}
...
function displayname_exists( $displayname, $user_id = 0 ) {
	if ( $user = get_user_by('displayname', $displayname ) ) {
		if  ( $user->ID != $user_id ) {
			return true;
		}
		else {
			return false; // ignore myself
		}
	}
...
/**
 * lookup the displayname in the signups.
 * TODO: performance may become a bottleneck if there're
 * too many signups since it uses REGEXP.
 * may need to add a display_name entry in signups
 */
function displayname_reserved( $display_name ) {
	global $wpdb;

	$signup = $wpdb->get_row ($wpdb->prepare ("SELECT * FROM $wpdb->signups WHERE signup_display_name = %s", $display_name) );
//	$signup = $wpdb->get_row( $wpdb->prepare("SELECT * FROM $wpdb->signups WHERE meta REGEXP %s", $display_name) );
	if ( ($signup != null) && (!$signup->active) ) {
		$registered_at =  mysql2date('U', $signup->registered);
		$now = current_time( 'timestamp', true );
		$diff = $now - $registered_at;
		// If registered more than two days ago, cancel registration and let this signup go through.
		if ( $diff > 2 * DAY_IN_SECONDS ) {
			$wpdb->delete( $wpdb->signups, array( 'user_login' => $signup->user_login ) );
			return false;
		}
		else {
			return true;

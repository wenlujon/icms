...
function wpmu_validate_user_signup($user_name, $user_email) {
	...
	//if ( $user_name != $orig_username || (preg_match( '/[^a-zA-Z0-9]/', $user_name ) && preg_match( '/[^\u4e00-\u9fa5]/', $user_name ) )) {
	...
	if ( is_email_address_unsafe( $user_email ) )
		$errors->add('user_email',  __('您不能使用此电子邮件地址注册。因为有不少恶意攻击者通过此类邮箱注册。请填写您的其它电子邮件地址。'));
...
*/
	...
	$display_name=sanitize_display_name($_POST['field_1'], true);
	//$display_name=$_POST['field_1'];
	// Check if the displayname has been used already.
	if ( displayname_exists($display_name) )
		$errors->add( 'display_name', __( '抱歉，此昵称已经被使用。' ) );
	...
	if ( displayname_reserved( $display_name ) ) {
		$errors->add('display_name', __('此昵称目前已经被占用，几天后也许可以使用。'));
	}
 ...
 */
function wpmu_signup_user( $user, $user_email, $meta = array(), $signup_display_name = '' ) {
	global $wpdb;
	...
	$meta = serialize($meta);
	$signup_display_name = sanitize_display_name( $signup_display_name );

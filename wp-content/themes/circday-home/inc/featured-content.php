...
/*
 * for somehow, the tag-name in database had the value '测试',
 * the query command is:
 * SELECT option_value FROM wp_options WHERE option_name LIKE '%featured_content%' LIMIT 1;
 * the following code is to recover the issue.
		if ($saved['tag-name'] == '测试') {
			$saved['tag-name'] = '特色';
			update_option('featured-content', $saved);
		}

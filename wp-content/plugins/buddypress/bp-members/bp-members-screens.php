...
function bp_core_screen_signup() {
	...
	// If the signup page is submitted, validate and save
		...
		if ( !empty( $account_details['errors']->errors['display_name'] ) ) {
			$bp->signup->errors['field_1'] = $account_details['errors']->errors['display_name'][0];
		}
		...
		if ( !empty( $bp->signup->errors ) ) {
			...
			if ( 'none' != $active_signup ) {
				...
				if ( bp_is_active( 'xprofile' ) ) {
					...
					foreach ( (array) $profile_field_ids as $field_id ) {
						...
						if ( !empty( $_POST['field_' . $field_id] ) )
							$usermeta['field_' . $field_id] = sanitize_display_name($_POST['field_' . $field_id]);
				...
				if ( isset( $_POST['signup_with_blog'] ) && is_multisite() )
					...
					$wp_user_id = bp_core_signup_user( $_POST['signup_username'], $_POST['signup_password'], $_POST['signup_email'], $usermeta, $_POST['field_1'] );

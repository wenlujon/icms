...
function bp_core_get_userid_from_displayname( $display_name = '' ) {
	if ( empty( $display_name ) ) {
		return false;
	}

	$user = get_user_by( 'displayname', $display_name );

	/**
	 * Filters the user ID based on display_name.
	 *
	 * @since BuddyPress (1.2.3)
	 *
	 * @param int|null $value         ID of the user or null.
	 ...
	 */
	return apply_filters( 'bp_core_get_userid_from_displayname', ! empty( $user->ID ) ? $user->ID : NULL, $display_name );
}

function bp_core_get_displayname( $user_id = 0, $display_name = false, $user_login = false ) {
	$bp = buddypress();
	...
	if ( false === $username ) {
		...
		if ( empty( $display_name ) && empty( $user_login ) ) {
		...
		$username = bp_is_username_compatibility_mode() ? $user_login : $display_name;
...
function bp_core_get_userlink_by_username( $username ) {
	if ( bp_is_username_compatibility_mode() ) {
		...
		$user_id = bp_core_get_userid_from_displayname( $username );
	}
 ...
 */
function bp_core_signup_user( $user_login, $user_password, $user_email, $usermeta, $signup_display_name = '' ) {
	$bp = buddypress();
	...
	if ( is_multisite() ) {
		wpmu_signup_user( $user_login, $user_email, $usermeta, $signup_display_name );

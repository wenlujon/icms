...
function bp_activity_at_name_filter( $content, $activity_id = 0 ) {
	...
	foreach ( $usernames as $username ) {
		...
		preg_match_all( '/(<a.*?(?!<\/a>)@' . $username . '.*?<\/a>)/u', $content, $content_matches );
	...
	foreach ( (array) $usernames as $user_id => $username ) {
		$content = preg_replace( '/(@' . $username . '\b)/u', "<a href='" . bp_core_get_user_domain( $user_id ) . "' rel='nofollow'>@$username</a>", $content );
	}
...
function bp_activity_at_name_filter_updates( $activity ) {
	...
	if ( ! empty( $usernames ) ) {
		...
		foreach( (array) $usernames as $user_id => $username ) {
			$activity->content = preg_replace( '/(@' . $username . '\b)/u', "<a href='" . bp_core_get_user_domain( $user_id ) . "' rel='nofollow'>@$username</a>", $activity->content );
		//printf(__('bolt-test, user_id:%s,username:%s'), $user_id, $username);
		}

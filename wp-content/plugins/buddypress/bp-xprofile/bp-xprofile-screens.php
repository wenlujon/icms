...
function xprofile_screen_edit_profile() {
	...
	$display_name=sanitize_display_name($_POST['field_1'], true);
		...
		else if ( displayname_exists( $display_name, bp_displayed_user_id() ) ) {
			bp_core_add_message( __( '此昵称已经被使用！', 'buddypress' ), 'error' );
		}
		else if ( displayname_reserved( $display_name ) ) {
			bp_core_add_message( __( '此昵称目前已经被占用，几天后也许可以使用。', 'buddypress' ), 'error' );
		} 
		else {

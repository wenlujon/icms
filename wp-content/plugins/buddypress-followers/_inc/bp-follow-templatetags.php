...
function bp_follow_add_follow_button( $args = '' ) {
	...
	function bp_follow_get_add_follow_button( $args = '' ) {
        ...
        $is_being_followed = bp_follow_is_following( array(
            'follower_id'   => $r['leader_id'],
            'leader_id' => $r['follower_id']
        ) );
		...
		if ( $is_following ) {
			...
			$action    = 'stop';
            $link_id   = 'unfollow';
            if (!empty($r['context']) && ($r['context'] == 'article') ) {
                $class     = 'bp_unfollow_button';
            }
            else {
                $class     = 'unfollow';
            }
		...
		$button = array(
			...
			'link_id'           => $link_id . '-' . (int) $r['leader_id'],

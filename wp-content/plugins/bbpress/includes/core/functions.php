...
function bbp_find_mentions_pattern() {
	//return apply_filters( 'bbp_find_mentions_pattern', '/[@]+([\s|\S]+)\b/u' );
	return apply_filters( 'bbp_find_mentions_pattern', '/[@]+([\x{4e00}-\x{9fa5}A-Za-z0-9-_\.@]+)\b/u' );
	//return apply_filters( 'bbp_find_mentions_pattern', '/[@]+([^\b]+)\b/u' );
}
...
function bbp_find_mentions( $content = '' ) {
	...
	//printf(__('content: <%s>'), $content);
	preg_match_all( $pattern, $content, $usernames );
	$usernames = array_unique( array_filter( $usernames[1] ) );
	//printf(__('usernames: <%s>'), $usernames);
...
function bbp_mention_filter( $content = '' ) {
	...
	//			printf(__('username:%s'), $username);

		// Skip if username does not exist or user is not active
		$user = get_user_by( 'displayname', $username );
		if ( empty( $user->ID ) || bbp_is_user_inactive( $user->ID ) )
			continue;

		// Replace name in content
		$content = preg_replace( '/(@' . $username . '\b)/u', sprintf( '<a href="%1$s" rel="nofollow">@%2$s</a>', bbp_get_user_profile_url( $user->ID ), $username ), $content );
	}

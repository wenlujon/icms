...
add_action('icms_home_icon', 'hefo_icms_home_icon');

function hefo_icms_home_icon() {
    global $hefo_options;

    if (!empty($hefo_options['home_icon'])) {
        $buffer = hefo_replace($hefo_options['home_icon']);
...
add_filter('icms_home_logo', 'hefo_icms_home_logo');

function hefo_icms_home_logo($html) {
    global $hefo_options;
	
    if (!empty($hefo_options['home_logo'])) {
        $buffer = hefo_replace($hefo_options['home_logo']);
...
add_action('icms_partnership', 'hefo_icms_partnership');

function hefo_icms_partnership() {
    global $hefo_options;

    if (!empty($hefo_options['partnership'])) {
        $buffer = hefo_replace($hefo_options['partnership']);
...
add_action('icms_credits', 'hefo_icms_credits');

function hefo_icms_credits() {
    global $hefo_options;

    if (!empty($hefo_options['credit'])) {
        $buffer = hefo_replace($hefo_options['credit']);
...
add_action('account_traffic', 'hefo_account_traffic');

function hefo_account_traffic() {
    global $hefo_options;
...
add_action('the_content', 'hefo_the_content_copyright');

function hefo_the_content_copyright($content) {
    global $hefo_options, $wpdb, $post, $hefo_page_top, $hefo_page_bottom, $hefo_post_top, $hefo_post_bottom, $hefo_is_mobile;
    ...
    if (is_singular()) {
        if ($hefo_post_bottom) {
            ...
            if ($value != '1') {
                    ...
                    if ($post_time >= 1483228800) {  // only 2017-1-1 onwards
                        $after = hefo_execute(hefo_replace($hefo_options['copyright_after']));
                    }

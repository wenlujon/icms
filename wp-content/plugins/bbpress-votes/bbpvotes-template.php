...
function bbpvotes_get_score_link( $args = '' ) {
        ...
        $r['text'] = sprintf(__('总分: %1$d','bbpvotes'),$score);
        $r['title'] = sprintf(__('打分人数: %1$d','bbpvotes'),$votes_count);
...
function bbpvotes_get_vote_up_link( $args = '' ) {
        ...
        switch($post_type){
            case bbp_get_topic_post_type():
                $r['title'] = __('此贴清晰而且有用','bbpvotes');
            break;
            case bbp_get_reply_post_type():
                $r['title'] = __('回帖有用','bbpvotes');
            break;
        }

        //check if user has already voted
        if ($voted_up = bbpvotes_has_user_voted_up_for_post( $post->ID )){
            $r['text'] = esc_html__( '您给了好评',   'bbpvotes' );
            $r['title'] = esc_html__( '您已经给了此贴好评',   'bbpvotes' );
...
function bbpvotes_get_vote_down_link( $args = '' ) {
        ...
        switch($post_type){
            case bbp_get_topic_post_type():
                $r['title'] = __('此贴不清晰而且没有意义','bbpvotes');
            break;
            case bbp_get_reply_post_type():
                $r['title'] = __('此回帖没有意义','bbpvotes');
            break;
        };
        
        //check if user has already voted
        if ($voted_down = bbpvotes_has_user_voted_down_for_post( $post->ID )){
            $r['text'] = esc_html__( '您给了差评',   'bbpvotes' );
            $r['title'] = esc_html__( '您给了此贴差评',   'bbpvotes' );
            $link_classes[] = 'bbpvotes-post-voted';
        }

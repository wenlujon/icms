...
echo ' <div class="register-section" style="clear:left; margin-top:-10px;">';

echo '<label style="width:345px;">';
echo '	请输入验证码，如果看不清，点击验证码图标即可刷新';
echo '</label>'; 
...
//echo '<label for="captcha_code">';
 ...
 // echo ($si_captcha_opt['si_captcha_label_captcha'] != '') ? $si_captcha_opt['si_captcha_label_captcha'] : __('CAPTCHA Code', 'si-captcha');
  //echo '</label>';
echo '<input class="register-form" style="margin-left:110px;width:145px;" id="captcha_code" name="captcha_code" type="text" value="" '.$si_aria_required.' />';
echo '</div> '; 
}
} // end function si_captcha_wpmu_signup_form
...
*/
  ...
  $si_html .= '      <img id="'.$label.'" class="captchaImgRefresh" src="'.$securimage_show_url.'" alt="';
  $si_html .= ($si_captcha_opt['si_captcha_tooltip_refresh'] != '') ? esc_attr( $si_captcha_opt['si_captcha_tooltip_refresh'] ) : esc_attr(__('Refresh Image', 'si-captcha'));

...
class Random_Images_Widget Extends \WP_Widget {
  ...
  function Get_Fancy_Images($orderby = 'ID', $order = 'ASC', $limit = -1){
    ...
    $stmt = "
      ...
      AND    gallery.post_type = \"".Core::$gallery_post_type->name."\"
  ...
  function Form ($settings){
    ...
    <p>
      ...
      <input type="number" name="<?php Echo $this->Get_Field_Name('thumb_width') ?>" id="<?php Echo $this->Get_Field_Id('thumb_width') ?>" value="<?php Echo Esc_Attr($this->Get_Option('thumb_width')) ?>"> px
    </p>

    <p>
      <label for="<?php Echo $this->Get_Field_Id('thumb_height') ?>"><?php Echo $this->t('Thumbnail height:') ?></label>
      <input type="number" name="<?php Echo $this->Get_Field_Name('thumb_height') ?>" id="<?php Echo $this->Get_Field_Id('thumb_height') ?>" value="<?php Echo Esc_Attr($this->Get_Option('thumb_height')) ?>"> px
    </p>
  ...
  function Widget ($args, $settings){
    ...
    Else {
      ...
      ForEach ($arr_images AS &$image){
        $include .= $image->ID . ',';
        If($this->Get_Option('link_target') == 'gallery')
          $image->href = Get_Permalink($image->gallery_id);
      }
    }
    ...
    $attributes = Core::Generate_Gallery_Attributes(Array(
      ...
      'columns' => Min(Count($arr_images), 3)
    ));

    # Build Gallery
    Core::Build_Gallery ($arr_images, $attributes);
    ...
    Echo $this->core->Render_Gallery($this->Get_Option('template'));

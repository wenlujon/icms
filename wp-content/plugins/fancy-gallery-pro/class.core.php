...
class Core {
  public
    ...
    $updates, # Pointer to the Updates object
  ...
  public static
    ...
    $version = '1.2.20', # Current release number
  ...
  function __construct($plugin_file){
    ...
    self::$gallery_post_type = New Gallery_Post_Type($this);
    If ($this->options->Get('lightbox') == 'on') $this->lightbox = new Lightbox($this);
    $this->updates = New Updates($plugin_file, $this->options->Get('update_username'), $this->options->Get('update_password'));
  ...
  public function t($text, $context = False){
    return $this->i18n->t($text, $context);
  }

  public function Plugin_Activation(){
    $this->i18n->Load_TextDomain();
    self::$gallery_post_type->Update_Post_Type_Name();
    self::$gallery_post_type->Register_Taxonomies();
    self::$gallery_post_type->Register_Post_Type();
    Flush_Rewrite_Rules();
  }
  ...
  private function Load_Base_Url(){
    ...
    Else
      self::$base_url = Plugins_Url(BaseName(DirName(__FILE__)));

    self::$base_url = Str_Replace("\\", '/', self::$base_url); # Windows Workaround
  }

  function Register_Widgets(){
    Register_Widget(SPrintF('%s\Random_Images_Widget', __NAMESPACE__));
    Register_Widget(SPrintF('%s\Taxonomies_Widget', __NAMESPACE__));
    Register_Widget(SPrintF('%s\Taxonomy_Cloud_Widget', __NAMESPACE__));
  }
  ...
  public function Enqueue_Frontend_Styles_and_Scripts(){
    ...
    If (!Current_Theme_Supports('html5', 'gallery')){
      $this->Enqueue_Frontend_StyleSheet(self::$base_url . '/assets/css/html5-galleries.css');
    }
    ...
    # Enqueue the Script
    WP_Enqueue_Script('fancy-gallery', self::$base_url . '/assets/js/fancy-gallery.js', Array('jquery'), self::$version, $this->options->Get('script_position') != 'header');
  ...
  public function Filter_Excerpt($excerpt){
    If ($GLOBALS['post']->post_type == self::$gallery_post_type->name && self::$gallery_post_type->Get_Meta('excerpt_type') == 'images' && !Post_Password_Required()){
      return $this->ShortCode_Gallery(Array(
        'number'          => self::$gallery_post_type->Get_Meta('excerpt_image_number'),
        'orderby'         => 'rand',
        'thumb_width'     => self::$gallery_post_type->Get_Meta('excerpt_thumb_width'),
        'thumb_height'    => self::$gallery_post_type->Get_Meta('excerpt_thumb_height'),
        'thumb_grayscale' => self::$gallery_post_type->Get_Meta('excerpt_thumb_grayscale'),
        'thumb_negate'    => self::$gallery_post_type->Get_Meta('excerpt_thumb_negate'),
        'template'        => self::$gallery_post_type->Get_Meta('excerpt_template')
      ));
    }
    Else {
      return $excerpt;
  ...
  public function Filter_Content($content){
    If ($GLOBALS['post']->post_type == self::$gallery_post_type->name && StrPos($content, '[gallery]') === False && StrPos($content, '[gallery ') === False && !Post_Password_Required()){
  ...
  static function Resampled_Image_URL($attachment_id, $width = 0, $height = 0, $crop = False, $grayscale = False, $negate = False, $qualy = 80){
    Global $blog_id;
    ...
    Else
      return self::$base_url . '/wp-thumb.php?' . HTTP_Build_Query(Array(
      ...
      If ($arr_properties = $this->Get_Template_Properties($template_file)){
        ...
        If ($stylesheet_url){
          If (StrPos($stylesheet_file, DirName(__FILE__)) === 0){ # the template is inside the plugin folder
            $stylesheet_url = self::$base_url . SubStr($stylesheet_file, StrLen(DirName(__FILE__)));
          }
  ...
  static function Generate_Gallery_Attributes($attributes){
    Global $post;
    ...
    If (IsSet($attributes['id']) && !Empty($attributes['id']))
      $gallery_meta = self::$gallery_post_type->Get_Meta(Null, False, $attributes['id']);
    Else {
      $attributes['id'] = $post->ID;
      If ($post->post_type == self::$gallery_post_type->name){
        $gallery_meta = self::$gallery_post_type->Get_Meta(Null, False, $attributes['id']);
      }
      Else {
        $gallery_meta = self::$gallery_post_type->Default_Meta();
      }
    }
  ...
  static function Build_Gallery($arr_images, $attributes){
    # Prepare Gallery Array
    self::$gallery = (Object) Array(
    ...
    If (!Empty($attributes['post_parent'])){ # this gallery uses the post attachments
      self::$gallery->id = $attributes['post_parent'];
    }
    Else { # this gallery only includes images
      self::$gallery->id = Sanitize_Title($attributes['include']);
    }
    ...
    ForEach ($arr_images AS $id => &$image){
      ...
      If ( $width != $attributes['thumb_width'] || $height != $attributes['thumb_height'] || $attributes['thumb_grayscale'] || $attributes['thumb_negate'] ){
        ...
        $src = self::Resampled_Image_URL($image->ID, $width, $height, True, ($attributes['thumb_grayscale'] == True), ($attributes['thumb_negate'] == True), 95);
      }
  ...
  public function ShortCode_Gallery ($attributes = Array()){
    $attributes = self::Generate_Gallery_Attributes($attributes);
    ...
    $arr_gallery = Get_Posts($attributes);
  	...
  	# Build the Gallery object
  	self::Build_Gallery($arr_gallery, $attributes);
  ...
  function Filter_Get_The_Categories($arr_categories){
    ...
    If (!Is_Admin()){
      ...
      $is_gallery = $post->post_type == self::$gallery_post_type->name;

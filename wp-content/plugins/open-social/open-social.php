...
function open_init() {
	...
	$GLOBALS['open_share_arr'] = array(
		'weibo'=>array(__('分享到微博','open-social'),"http://v.t.sina.com.cn/share/share.php?url=%URL%&title=%TITLE%&pic=%PIC%&appkey=".osop('SINA_AKEY')."&ralateUid=".osop('share_sina_user')."&language=zh_cn&searchPic=false"),
//		'qqzone'=>array(__('Share with QQZone','open-social'),"http://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_onekey?url=%URL%&title=%TITLE%&desc=&summary=&site=&pics=%PIC%"),
		'wechat'=>array(__('扫二维码分享到微信','open-social'),"QRCODE"),
		'qqzone'=>array(__('分享到QQ空间', 'open-social'),"http://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_onekey?url=%URL%"),
		...
		'qqchat'=>array(__('分享到QQ好友', 'open-social'), 'http://connect.qq.com/widget/shareqq/index.html?url=%URL%&desc=&title=%TITLE%&summary=&pics=%PIC%&flash=&site=&style=201&width=32&height=32&showcount='),
...
class SINA_CLASS {
	...
	function open_new_user(){
		...
		return array(
			...
			'user_email' => strtoupper(OPEN_TYPE).$_SESSION['open_id'].'@example.com'
		);
	} 
} 
...
class BAIDU_CLASS {
	...
	function open_new_user(){
		...
		return array(
			...
			'user_email' => strtoupper(OPEN_TYPE).$user["uid"].'@example.com'
		);
	}
} 
...
class GITHUB_CLASS {
	...
	function open_new_user(){
		...
		return array(
			...
			'user_url' => $user['url'],
			'user_email' => strtoupper(OPEN_TYPE).$user['id'].'@example.com'
		);
	} 
} 
...
function open_action($os){
	...
	if (is_user_logged_in()) { //bind
		...
		if (!$wpuid) {
			$candidate_username = uniqid(OPEN_TYPE, false);
			$create_username = $candidate_username;

			while (username_exists($create_username)) {
				$username_suffix = substr(str_shuffle("0123456789abcdefghijklmnopqrstuvwxyz"), 0, 3);
				$create_username = $candidate_username . $username_suffix;
			}

			$getnickname = sanitize_display_name($getnickname); // in case some nickname has white space in between
			$create_display_name = $getnickname;
			
			while (displayname_exists($create_display_name)) {
				$create_name_suffix = substr(str_shuffle("0123456789abcdefghijklmnopqrstuvwxyz"), 0, 3);
				$create_display_name = $getnickname. $create_name_suffix;
			}

			$userdata = array(
				'nickname' => $create_display_name,
				'display_name' => $create_display_name,
				'user_url' => '',
				'user_email' => $create_username.'@example.com',
				'user_pass' => wp_generate_password(),
				'user_login' => $create_username
			);
...
function open_connect_http($url, $postfields='', $method='GET', $headers=array()){
	...
	$headers[] = 'User-Agent: '. DOMAIN_CURRENT_SITE;
	$headers[] = 'Expect:';
...
//login and share
if( osop('show_login_page',1) ) add_action('login_form_social', 'open_social_login_form');
//if( osop('show_login_page',1) ) add_action('bp_before_register_page', 'open_social_login_form');
...
if( osop('show_share_content',1) ) {
	add_filter('the_tags', 'open_social_share_form');
	function open_social_share_form($content) {
		if(is_single()) {
			$share_content = open_social_share_html();
			return  $content.$share_content;
		}
		return $content;
	}
}
...
function open_login_button_show($icon_type,$icon_title,$icon_link){
	...
	$home_dir=home_url();
	return "<a href=\"#\" onclick=\"login_button_click('$icon_type','$icon_link')\"> <img src=\"$home_dir/images/qq_login.png\"></a>";
}
